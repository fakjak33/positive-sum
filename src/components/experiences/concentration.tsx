...
export function Concentration() {
  ...
  const shown = sector
    ? constituents.filter((c) => c.sector === sector)
    : constituents;

  const top10 = constituents.slice(0, 10);
  const top10Contribution =
    top10.reduce((a, c) => a + c.return, 0) /
    constituents.reduce((a, c) => a + c.return, 0);

  return (
    <ExperienceFrame
      title={`Where the ${constituentsYear} return actually came from`}
    ...
    >
      ...
      <DataTable
        caption={`S&P 500 constituent returns by sector, ${constituentsYear}.`}
